    ...
    class Completing {
        ...
        void provider_shouldTransitionToDeprovisioning_whenSendingMessageSucceed() {
            ...
            await().untilAsserted(() -> {
                ...
                verify(transferProcessStore, atLeastOnce()).save(argThat(p -> p.getState() == DEPROVISIONING.code() && p.getLastPreviousState() == COMPLETED.code()));
    ...
    class TerminatingProvider {
        ...
        void shouldTransitionToDeprovisioning_whenMessageSentCorrectly() {
            ...
            await().untilAsserted(() -> {
                ...
                assertThat(message.getConsumerPid()).isEqualTo("correlationId");
                assertThat(message.getProcessId()).isEqualTo("correlationId");
                verify(transferProcessStore, atLeastOnce()).save(argThat(p -> p.getState() == DEPROVISIONING.code() && p.getLastPreviousState() == TERMINATED.code()));
